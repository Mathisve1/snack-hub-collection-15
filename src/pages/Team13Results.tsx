...
import { ArrowLeft, Table as TableIcon, LayoutGrid, AlertTriangle } from "lucide-react";
...
const Team13Results = () => {
  ...
  useEffect(() => {
    console.log("Team13Results - Data state:", {
      personas: {
    ...
    if (!isLoading && !hasErrors) {
      if ((personas?.length || 0) > 0) {
        toast.success(`Loaded ${personas?.length} Team 13 buying personas`);
      } else if (!personasLoading && (personas?.length || 0) === 0) {
        toast.warning("No Team 13 buying personas data found");
      }
    }
            ...
            <div className="p-6">
              ...
              {!isLoading && !hasErrors && 
               personas.length === 0 && frituren.length === 0 && interviews.length === 0 && (
                <Alert className="mb-6 border-amber-400 bg-amber-50 text-amber-800">
                  ...
                  <AlertDescription>
                    No data found in Team 13 tables. Please check that the Team 13 tables exist and contain data.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20">
                  ...
                  <p className="text-gray-500">Loading Team 13 research data...</p>
                </div>
              ) : hasErrors ? (
                <div className="text-red-500 p-6 text-center">
                  <p>There was a problem loading the Team 13 research data. Please try again later.</p>
